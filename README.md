...
Firebase powers **push notifications** (FCM) for the app. Both the backend server and the Flutter app need to be configured.

---

### Backend (Server Deployment)

The backend uses the **Firebase Admin SDK** (`firebase-admin`) to send FCM push notifications to devices. It reads a **Service Account JSON file** at startup. This is the most critical step for push notifications to work on your production server.
...
3. A JSON file will download. It contains your private credentials and looks like this:
...
The backend code in `notification/fcm.py` looks for the file at this **exact path** relative to the backend root:

```
ems-full-stack/
└── backend/
    └── firebase-service-account.json   ← must be placed here
```

Upload it from your local machine to the server using `scp`:

```bash
scp /path/to/downloaded-key.json user@your-server-ip:/path/to/ems-full-stack/backend/firebase-service-account.json
```

> **Never commit this file to Git.** It contains your private key and grants full Firebase Admin access.
> It is already listed in `.gitignore` — always transfer it manually to the server.

#### Step 4 — Secure File Permissions on the Server

```bash
# Only the app process user should be able to read it
chmod 600 /path/to/backend/firebase-service-account.json
chown www-data:www-data /path/to/backend/firebase-service-account.json
```

#### How the Backend Uses It

The file is loaded once at startup in `notification/fcm.py`:

```python
cred_path = os.path.join(os.path.dirname(...), 'firebase-service-account.json')
cred = credentials.Certificate(cred_path)
firebase_admin.initialize_app(cred)
```

Every time a notification is triggered (calendar event fires, leave approved, task assigned), the backend:
1. Saves the notification to the **database** (for the in-app bell icon).
2. Sends a real **FCM push** to all registered device tokens for that user via `send_push_notification()`.

> **Warning:** If `firebase-service-account.json` is missing on the server, the first push notification attempt will throw a `FileNotFoundError`. The app will still work, but **no push notifications will be delivered**.

---

### Frontend (Flutter App)

#### Step 1 — Register the App in Firebase Console
1. In Firebase Console → **Project Overview** → **Add App** → select **Android**.
2. Enter the package name from `frontend/android/app/build.gradle` (`applicationId`).
...
└── android/
    └── app/
        └── google-services.json    ← required for Android FCM
```

#### Step 3 — AndroidManifest.xml Entries
The following are already in `frontend/android/app/src/main/AndroidManifest.xml`:
```xml
<!-- Google Maps API Key -->
<meta-data android:name="com.google.android.geo.API_KEY"
           android:value="YOUR_MAPS_API_KEY"/>

<!-- Default notification icon (white silhouette — required by Android) -->
<meta-data
    android:name="com.google.firebase.messaging.default_notification_icon"
    android:resource="@drawable/ic_stat_notify" />
```

#### Step 4 — Device Token Registration Flow
On login, the app automatically:
1. Fetches the device FCM token from Firebase.
2. POSTs it to `POST /api/notifications/device-token/`.
3. The backend stores it in the `DeviceToken` table and uses it for all future pushes.
