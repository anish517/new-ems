...
class _SalaryScreenState extends ConsumerState<SalaryScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final salaries = salaryRes.data is List ? salaryRes.data as List : ((salaryRes.data['results'] ?? []) as List);

      // 2. Fetch employees (for mapping names)
      final empRes = await ApiService().get('/api/organization/employees/');
      final employees = empRes.data is List ? empRes.data as List : ((empRes.data['results'] ?? []) as List);

      // 3. Admin transactions
      if (isAdmin) {
        final txRes = await ApiService().get('${AppConstants.salaryBase}/transactions/organization/');
        _allTransactions = txRes.data is List ? txRes.data as List : ((txRes.data['results'] ?? []) as List);
      ...
      if (empId != null) {
        ...
        if (_mySalary != null) {
          ...
          try {
            ...
            _myTransactions = txRes.data is List ? txRes.data as List : ((txRes.data['results'] ?? []) as List);
          } catch (_) {}
        }
      }
    ...
    return Scaffold(
      ...
      floatingActionButton: isAdmin
          ? FloatingActionButton.extended(
              ...
              label: const Text('Issue Salary', style: TextStyle(color: Colors.white)),
            )
          : null,
    );
  }
  ...
  void _showEmployeeSalaryDetail(
      BuildContext ctx, Map salary, int empId) {
      ...
      child: _allTransactions.isEmpty
          ...
          : ListView.separated(
              ...
              itemBuilder: (ctx, i) {
                ...
                return _TransactionTile(tx, empName: _empName(tx['employee'] ?? 0));
              },
            ),
    );
  }
  ...
  Widget _buildEmployeeView() {
    if (_mySalary == null) {
      return const Center(child: Text('No salary record found.\nContact your admin.', textAlign: TextAlign.center));
    }
    return RefreshIndicator(
      onRefresh: _loadData,
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(_fmt(_myNetSalary?['net_salary'] ?? _mySalary?['basic_salary']),
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                _PayslipItem('Net', _fmt(_myNetSalary?['net_salary'] ?? _mySalary?['basic_salary'])),
              ]),
            ]),
          ),
      ...
      builder: (sheetCtx) => StatefulBuilder(builder: (context, setState) {
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Text('Edit Base Salary for ${_empName(salaryRecord['employee'])}',
            ...
            TextField(
              ...
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'Remote Salary (NPR)'),
            ),
            const SizedBox(height: 24),
            ElevatedButton(
...
class _CreateSalarySheet extends StatefulWidget {
  ...
  const _CreateSalarySheet({required this.salaries, required this.employees, required this.onCreated});
  @override
...
class _CreateSalarySheetState extends State<_CreateSalarySheet> {
  ...
  int? _selSalaryId;
  int? _selFiscalYear;
  // Default to today in YYYY-MM-DD (server parses as Nepali date)
  String _date = () {
  ...
  Future<void> _loadNetSalary() async {
    ...
    try {
      final res = await ApiService().get('/api/salary-management/net-salary/$_selSalaryId/?date=$_date');
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          const Text('Issue Salary', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          if (widget.salaries.isEmpty)
            ...
            DropdownButtonFormField<int>(
              ...
              isExpanded: true,
              items: widget.salaries.fold<List<dynamic>>([], (prev, curr) {
                if (!prev.any((e) => e['id'] == curr['id'])) prev.add(curr);
                return prev;
              }).map((s) => DropdownMenuItem<int>(
                value: s['id'], child: Text('${_empName(s['employee'])} (Base: ${s['basic_salary']})'),
              )).toList(),
              onChanged: (v) { setState(() => _selSalaryId = v); _loadNetSalary(); },
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<int>(
              ...
              items: _fiscalYears.isEmpty
                ? [const DropdownMenuItem(value: -1, child: Text('No fiscal years available'))]
                : _fiscalYears.fold<List<dynamic>>([], (prev, curr) {
                    if (!prev.any((e) => e['id'] == curr['id'])) prev.add(curr);
                    return prev;
            ...
            TextFormField(
              ...
              onChanged: (v) { _date = v; if(v.length == 10) _loadNetSalary(); },
            ),
            const SizedBox(height: 16),
            if (_netSalaryInfo != null)
              Container(
                ...
                child: Column(children: [
                  _InfoRow(Iconsax.calendar, 'Days Present', '${_netSalaryInfo!['no_of_days_present']}'),
                  _InfoRow(Iconsax.calendar_remove, 'Paid Leaves', '${_netSalaryInfo!['paid_leaves']}'),
                  const Divider(color: Colors.white24),
                  _InfoRow(Iconsax.money_recive, 'Net Salary Payable', 'NPR ${_netSalaryInfo!['net_salary']}', bold: true),
                ]),
              ),
            const SizedBox(height: 24),
            ElevatedButton(
              onPressed: (_saving || _selSalaryId == null || _selFiscalYear == null) ? null : _save,
              child: _saving ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Issue Payment'),
            ),
          ]),
        ],
      ),
...
class _EmployeeSalaryDetailSheetState
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() { _netInfo = res.data; _loading = false; });
    } catch (e) {
      if (mounted) setState(() {
        _error = ApiService.getErrorMessage(e);
        _loading = false;
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_loading)
            ...
            _InfoRow(Iconsax.money_recive, 'Net Salary (This Month)',
                _fmt(_netInfo!['net_salary']), bold: true),
          ],
          const SizedBox(height: 24),
        ]),
      ),
