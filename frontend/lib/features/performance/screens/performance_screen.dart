        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: score >= 8 ? AppColors.success.withValues(alpha: 0.15)
                     : score >= 5 ? AppColors.warning.withValues(alpha: 0.15)
                     : AppColors.error.withValues(alpha: 0.15),
                borderRadius: BorderRadius.circular(12),
              ),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Iconsax.star1, size: 14,
                     color: score >= 8 ? AppColors.success : score >= 5 ? AppColors.warning : AppColors.error),
                const SizedBox(width: 4),
                Text('$score / 10', style: TextStyle(
                     color: score >= 8 ? AppColors.success : score >= 5 ? AppColors.warning : AppColors.error,
          ...
          if (hasReply) ...[
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(review['reply'], style: const TextStyle(fontSize: 14, color: AppColors.textSecondary)),
              ]),
            ),
          ] else if (!isAdmin) ...[
            Align(
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            TextField(
              controller: ctrl,
              maxLines: 4,
              decoration: const InputDecoration(labelText: 'Your response', alignLabelWithHint: true),
            ),
            ...
            ElevatedButton(
              onPressed: saving ? null : () async {
                ...
                try {
                  await ApiService().patch('/api/performance/reviews/${review['id']}/reply/', data: {
              ...
              child: saving ? const CircularProgressIndicator() : const Text('Submit Reply'),
            )
          ]),
        );
      }),
...
class _CreateReviewSheetState extends State<_CreateReviewSheet> {
  ...
  Future<void> _submit() async {
    if (_selEmployee == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select an employee', style: TextStyle(color: Colors.white)), backgroundColor: AppColors.error));
      return;
    }
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: ${ApiService.getErrorMessage(e)}'), backgroundColor: AppColors.error));
    } finally {
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Row(children: [
            ...
            Text('${_score.toInt()}', style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold, fontSize: 18)),
          ]),
          Slider(
            value: _score, min: 1, max: 10, divisions: 9,
            activeColor: _score >= 8 ? AppColors.success : _score >= 5 ? AppColors.warning : AppColors.error,
            onChanged: (v) => setState(() => _score = v),
          ),
          const SizedBox(height: 12),
          TextField(controller: _feedbackCtrl, maxLines: 3, decoration: const InputDecoration(labelText: 'Feedback', alignLabelWithHint: true)),
          const SizedBox(height: 12),
          TextField(controller: _suggCtrl, maxLines: 3, decoration: const InputDecoration(labelText: 'Suggestion/Goals', alignLabelWithHint: true)),
          const SizedBox(height: 24),
          ElevatedButton(
            onPressed: _saving ? null : _submit,
            child: _saving ? const CircularProgressIndicator() : const Text('Submit Review'),
          )
        ]),
      ),
    );
